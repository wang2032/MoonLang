...
from moonlang.bench_serving import run_benchmark
...
from moonlang.test.run_eval import run_eval
from moonlang.utils import get_exception_traceback, normalize_base_url
...
def select_sglang_backend(args: argparse.Namespace):
    from moonlang.lang.backend.openai import OpenAI
...
def _try_enable_offline_mode_if_cache_complete(
    ...
    """
    from moonlang.srt.model_loader.ci_weight_validation import (
        _get_per_run_marker_path,
        _read_per_run_marker,
        _write_per_run_marker,
        validate_cache_lightweight,
    )
    from moonlang.srt.utils import find_local_repo_dir
...
def popen_launch_server(
    ...
    if pd_separated or use_mixed_pd_engine:
        command = "moonlang.launch_pd_server"
    else:
        command = "moonlang.launch_server"
...
def run_bench_offline_throughput(model, other_args):
    command = [
        "python3",
        "-m",
        "moonlang.bench_offline_throughput",
        "--num-prompts",
        "1",
        "--dataset-name",
        "random",
...
def run_bench_one_batch_server(
    ...
    from moonlang.bench_one_batch_server import run_benchmark
    ...
    try:
        ...
        if base_path:
            try:
                ...
                logging.warning(
                    f"moonlang.test.dump_metric: failed to write to {jsonl_path}: {e}"
                )
        ...
        logging.warning(
            f"moonlang.test.dump_metric: failed to dump metric '{metric_name}': {e}",
            exc_info=True,
        )
