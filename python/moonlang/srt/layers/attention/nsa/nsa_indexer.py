...
if is_npu():
    import torch_npu
    from moonlang.srt.hardware_backend.npu.utils import get_indexer_weight_stream
...
from moonlang.srt.layers.linear import ReplicatedLinear
...
if TYPE_CHECKING:
    from moonlang.srt.mem_cache.memory_pool import NSATokenToKVPool
...
def rotate_activation(x: torch.Tensor) -> torch.Tensor:
    ...
    if _is_hip:
        ...
        from moonlang.jit_kernel.hadamard import hadamard_transform
...
class Indexer(MultiPlatformOp):
    ...
    def forward_indexer(
        ...
        if not _is_npu:
            from moonlang.srt.layers.attention.nsa.tilelang_kernel import fp8_index
    ...
    def forward_cuda(
        ...
        if _is_hip:
            from moonlang.srt.layers.attention.nsa.tilelang_kernel import act_quant
        elif not _is_npu:
            from moonlang.srt.layers.attention.nsa.triton_kernel import act_quant
