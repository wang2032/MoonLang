...
if TYPE_CHECKING:
    ...
    from moonlang.srt.layers.attention.base_attn_backend import AttentionBackend
    from moonlang.srt.model_executor.model_runner import ModelRunner
...
def create_flashinfer_backend(runner):
    ...
    if not runner.use_mla_backend:
        from moonlang.srt.layers.attention.flashinfer_backend import FlashInferAttnBackend
        ...
        from moonlang.srt.layers.attention.flashinfer_mla_backend import (
...
def create_trtllm_mla_backend(runner):
    ...
    from moonlang.srt.layers.attention.trtllm_mla_backend import TRTLLMMLABackend
...
def create_aiter_backend(runner):
    from moonlang.srt.layers.attention.aiter_backend import AiterAttnBackend
...
def create_wave_backend(runner):
    from moonlang.srt.layers.attention.wave_backend import WaveAttnBackend
...
def create_ascend_backend(runner):
    from moonlang.srt.hardware_backend.npu.attention.ascend_backend import (
        AscendAttnBackend,
    )
...
def create_nsa_backend(runner):
    from moonlang.srt.layers.attention.nsa_backend import NativeSparseAttnBackend
...
def create_triton_backend(runner):
    ...
    if runner.server_args.enable_double_sparsity:
        from moonlang.srt.layers.attention.double_sparsity_backend import (
            DoubleSparseAttnBackend,
        )

        return DoubleSparseAttnBackend(runner)
    else:
        from moonlang.srt.layers.attention.triton_backend import TritonAttnBackend
...
def create_torch_native_backend(runner):
    from moonlang.srt.layers.attention.torch_native_backend import TorchNativeAttnBackend
...
def create_flex_attention_backend(runner):
    from moonlang.srt.layers.attention.torch_flex_backend import TorchFlexAttnBackend
...
def create_flashmla_backend(runner):
    from moonlang.srt.layers.attention.flashmla_backend import FlashMLABackend
...
def create_flashattention_v3_backend(runner):
    ...
    from moonlang.srt.layers.attention.flashattention_backend import FlashAttentionBackend
...
def create_cutlass_mla_backend(runner):
    from moonlang.srt.layers.attention.cutlass_mla_backend import CutlassMLABackend
...
def create_trtllm_mha_backend(runner):
    ...
    from moonlang.srt.layers.attention.trtllm_mha_backend import TRTLLMHAAttnBackend
...
def create_intel_amx_backend(runner):
    from moonlang.srt.layers.attention.intel_amx_backend import IntelAMXAttnBackend
...
def create_dual_chunk_flash_attn_backend(runner):
    from moonlang.srt.layers.attention.dual_chunk_flashattention_backend import (
...
def attn_backend_wrapper(runner: "ModelRunner", full_attn_backend: "AttentionBackend"):
    ...
    if cfg := runner.mambaish_config:
        from moonlang.srt.layers.attention.fla.utils import check_environments
        from moonlang.srt.layers.attention.hybrid_linear_attn_backend import (
            HybridLinearAttnBackend,
            Mamba2AttnBackend,
        )
        from moonlang.srt.layers.attention.linear.gdn_backend import GDNAttnBackend
        from moonlang.srt.layers.attention.linear.kda_backend import KDAAttnBackend
        from moonlang.srt.layers.attention.linear.lightning_backend import (
            LightningAttentionBackend,
        )
        from moonlang.srt.layers.attention.linear.utils import (
            initialize_linear_attn_config,
        )
        from moonlang.srt.utils import is_blackwell, is_npu
...
def create_intel_xpu_backend(runner):
    from moonlang.srt.layers.attention.xpu_backend import XPUAttentionBackend
